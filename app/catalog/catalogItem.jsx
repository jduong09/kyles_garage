...
const CatalogItem = ({ cart, setCart, item, idx, deleteItem }) => {
  ...
  return (
    <div className="flex flex-col justify-start items-start p-3 bg-gray-400/20 rounded-lg">
      ...
      <button className={`self-end items-center rounded-full w-8 h-8 bg-yellow-300 hover:bg-yellow-600 px-1 py-1 font-medium dark:bg-orange-600 dark:hover:bg-orange-900 ${isOpen ? 'rotate-45' : ''}`} onClick={() => toggleAvailability()}>+</button>
      <div className={isOpen ? '' : 'hidden'}>
        <form className="flex flex-col">
          ...
          <button type="submit" className="items-center rounded-full bg-blue-600 hover:bg-blue-700 px-2 py-1 font-medium dark:bg-blue-500" onClick={(e) => handleAddCart(e)}>Add To Cart</button>
        </form>
      </div>
    </div>
  );
