...
const CatalogItem = ({ cart, setCart, item, idx, deleteItem }) => {
  ...
  const handleAddCart = (e) => {
    ...
    setCart([...cart, { inventory_uuid: item.inventory_uuid, name: item.name, price: item.price, finalPrice: Number((item.price / 100) * numOfReservedDays).toFixed(2), startDate, endDate, days: numOfReservedDays }]);
    setInputStart('');
    setInputEnd('');
    toggleAvailability();
  }
